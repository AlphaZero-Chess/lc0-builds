...
(function() {
    ...
    const CONFIG = {
        ...
        cPuct: null,                 // Not used in minimax mode
        useMCTS: false,              // Use fast minimax with anti-blunder
        
        // Time management (Optimized for speed)
        useSmartTiming: false,       // Use fixed timing for consistent speed
        minThinkTime: 500,           // Minimum thinking time (ms)
        maxThinkTime: 1500,          // Maximum thinking time (ms) - REDUCED
    ...
    const EngineManager = {
        ...
        onUciOk() {
            Logger.success('UCI protocol initialized - Minimax mode with anti-blunder');
            
            // No special options needed for minimax engine
            // Anti-blunder is built-in via SEE
        ...
        analyzePosition(fen) {
            ...
            // Send go command (use movetime for speed)
            let goCmd = `go movetime ${thinkTime}`;
            if (CONFIG.depth !== null) {
                goCmd += ` depth ${CONFIG.depth}`;
            }
    ...
    // ═══════════════════════════════════════════════════════════════════════
    function initialize() {
        Logger.banner('⚡ Lichess Lc0 Bot - Fast Anti-Blunder Edition v2.0');
        console.log('%c⚠️  EDUCATIONAL USE ONLY - DO NOT USE IN LIVE GAMES', 'color: #F44336; font-size: 16px; font-weight: bold;');
        console.log('');
        console.log('%c⚡ Fast Minimax + Anti-Blunder SEE System', 'color: #2196F3; font-size: 14px; font-weight: bold;');
        console.log('%c🎯 Expected Strength: ~1800-2100 ELO | Speed: 0.5-1.5s per move', 'color: #4CAF50; font-size: 14px; font-weight: bold;');
        console.log('');
