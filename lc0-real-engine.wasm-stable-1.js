...
window.LEELA = (function() {
    ...
    // Simple blunder prevention (keeps performance stable!)
    function avoidObviousBlunder(board, move) {
        ...
        // RULE: Don't move Queen to take small piece if obviously attacked
        if (attackerType === 5 && victimValue < 400) { // Queen taking N/B/P
            // Quick check: is destination attacked by pawn/knight?
            const toRank = Math.floor(move.to / 8);
            const toFile = move.to % 8;
            const enemyColor = -board.turn;
            
            // Check for enemy pawns attacking the square
    ...
    class SearchEngine {
        ...
        alphaBeta(board, depth, alpha, beta, isMaximizing) {
            ...
            // Filter obvious blunders at root only (minimal performance impact)
            if (depth === this.currentDepth) {
                const safeMoves = moves.filter(m => avoidObviousBlunder(board, m));
