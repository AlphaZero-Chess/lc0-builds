...
window.LEELA = (function() {
    ...
    class Board {
        ...
        // SUPER SIMPLE attack detection - just check adjacent squares
        isSquareAttacked(sq, byColor) {
            const rank = Math.floor(sq / 8);
            const file = sq % 8;

            // Just check if any enemy piece is adjacent or nearby
            for (let dr = -2; dr <= 2; dr++) {
                for (let df = -2; df <= 2; df++) {
                    ...
                    const isEnemy = (byColor === 1 && this.isWhite(piece)) || 
                                   (byColor === -1 && this.isBlack(piece));
                    if (!isEnemy) continue;
                    
                    // If Queen or Rook within 2 squares, consider attacked
                    const type = this.getPieceType(piece);
                    if (type === 5 || type === 4) return true; // Queen or Rook
                    
                    // If Bishop on diagonal within 2, consider attacked
                    if (type === 3 && Math.abs(dr) === Math.abs(df)) return true;
                    
                    // If Knight at knight distance, consider attacked
                    if (type === 2) {
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // SUPER SIMPLE BLUNDER CHECK - NO HEAVY CALCULATIONS
    // ═══════════════════════════════════════════════════════════════════════
    class SimpleBlunderCheck {
        static isMoveSafe(board, move) {
            ...
            // RULE 1: Never hang Queen for less (super simple check)
            const attackerType = board.getPieceType(attacker);
            if (attackerType === 5) { // Queen
                if (victimValue < 500) { // Taking anything less than Rook
                    // Quick check: is destination attacked?
                    if (board.isSquareAttacked(move.to, -board.turn)) {
                        return false; // Don't move Queen to attacked square for small piece
                    }
                }
            }
            
            // RULE 2: For captures, basic MVV check
            if (victim !== PIECES.EMPTY) {
                // Don't take pawn with Queen if square is attacked
                if (attackerType === 5 && victimValue <= 100) {
                    if (board.isSquareAttacked(move.to, -board.turn)) {
                        return false;
                    }
                }
                
                // Don't take if we lose way more than we gain
                if (board.isSquareAttacked(move.to, -board.turn)) {
                    if (attackerValue > victimValue + 200) {
                        return false; // Losing too much material
                    }
                }
            }
            
            return true; // Let most moves through
        }
    }
    ...
    class SearchEngine {
        ...
        alphaBeta(board, depth, alpha, beta, isMaximizing) {
            ...
            // SIMPLE: Filter out obvious blunders only at root
            if (SEE_ENABLED && depth === this.currentDepth) {
                const safeMoves = moves.filter(move => SimpleBlunderCheck.isMoveSafe(board, move));
                if (safeMoves.length > 0) {
                    moves = safeMoves; // Use safe moves if available
                }
            }
    ...
    class RealLc0Engine {
        ...
        processCommand(cmd) {
            if (cmd === 'uci') {
                this.output('id name Lc0 Super Lightweight v3.0');
        ...
        handleGo(cmd) {
            ...
            if (movetimeMatch) {
                timeLimit = Math.min(parseInt(movetimeMatch[1]), 1500); // Cap at 1.5s
            }

            const depthMatch = cmd.match(/depth\s+(\d+)/);
            if (depthMatch) {
                maxDepth = Math.min(parseInt(depthMatch[1]), 12); // Cap at 12
                timeLimit = Math.min(timeLimit, 2000); // Cap at 2s for depth
            }
    ...
    return function createLeelaEngine() {
        ...
        console.log('%c║  ⚡ Lc0 Super Lightweight Engine v3.0                ║', 'color: #4CAF50; font-weight: bold;');
        console.log('%c║  ✓ Fast minimax (depth 8, 0.8s moves)               ║', 'color: #4CAF50;');
        console.log('%c║  ✓ Simple blunder prevention (Queen protection)     ║', 'color: #4CAF50;');
        console.log('%c║  ✓ Browser compatible (no freeze!)                  ║', 'color: #4CAF50;');
        console.log('%c║  Expected Rating: ~1600-1900 ELO (Fast + Safe)      ║', 'color: #4CAF50;');
        console.log('%c╚═══════════════════════════════════════════════════════════╝', 'color: #4CAF50; font-weight: bold;');
...
console.log('%c✓ Super Lightweight Engine - Browser Compatible!', 'color: #4CAF50; font-weight: bold; font-size: 14px;');
