// ==UserScript==
// @name         Lichess Lc0 Bot - AlphaZero MCTS Fast Edition
// @description  Professional AlphaZero chess engine with MCTS+PUCT - optimized for speed
// @author       Claude AI
// @version      3.0.0
// @match        *://lichess.org/*
...
// NOTE: The engine is embedded below instead of using @require to avoid CORS issues
...
(function() {
    ...
    // ═══════════════════════════════════════════════════════════════════════
    const CONFIG = {
        // Engine settings (AlphaZero MCTS) - OPTIMIZED FOR SPEED
        movetime: 1000,              // Fast moves - 1 second (reduced from 2000ms)
        nodes: null,                 // Node count (null = use movetime)
        simulations: 400,            // MCTS simulations per move (reduced from 1600 for speed)
        depth: null,                 // Search depth (null = unlimited)
        multiPV: 1,                  // Number of lines to analyze
        threads: 2,                  // CPU threads
        temperature: 0.0,            // Move randomness (0.0 = deterministic for speed)
    ...
    // ═══════════════════════════════════════════════════════════════════════
    function initialize() {
        Logger.banner('🏆 Lichess Lc0 Bot - AlphaZero MCTS Edition v3.0');
